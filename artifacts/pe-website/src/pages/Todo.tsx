...
const delivered: ChecklistGroup[] = [
  ...
  },
  {
    ...
    items: [
      { done: true, title: "Per-page <title> tags tuned to each audience and route" },
      { done: true, title: "Per-page meta descriptions written for click-through, not keyword-stuffing" },
      { done: true, title: "Canonical URLs set on every page to prevent duplicate-content penalties" },
      { done: true, title: "Open Graph tags (title, description, image, url, type, site_name) on every page so LinkedIn / Slack / iMessage / Telegram unfurl correctly" },
      { done: true, title: "Twitter card tags (summary_large_image) for X / Twitter previews" },
      { done: true, title: "1200x630 social preview card with the L&E wordmark and tagline" },
      { done: true, title: "JSON-LD ProfessionalService structured data: firm name, founding date, full NYC address, phone, email, principals, sectors covered - tells Google exactly what L&E is" },
      { done: true, title: "robots.txt with sitemap reference and a Disallow for /todo" },
      { done: true, title: "sitemap.xml listing all 9 public routes with priorities and lastmod dates" },
      { done: true, title: "301-style permanent redirects from every old WordPress URL (/about-us, /contact-us, /services, /for-the-private-equity-firms, /for-the-executives) to the new equivalent so search-engine ranking transfers cleanly" },
      { done: true, title: "/todo and /404 explicitly set to noindex, nofollow" },
      { done: true, title: "Mobile viewport accessibility fix (removed maximum-scale=1 lockout)" },
      { done: true, title: "Cache-Control headers on /brand/, /le/, robots.txt, sitemap.xml, manifest" },
      { done: true, title: "format-detection telephone=no to prevent iOS auto-styling phone numbers as links inline" },
    ],
  },
  {
    id: "brand",
