...
const principles = [
  {
    ...
    desc: "We make fewer, larger bets — concentrating capital where our proprietary insight is most pronounced.",
  },
  {
    icon: <Layers className="w-6 h-6" />,
    title: "Operational Depth",
    desc: "Our value-creation teams embed alongside management to drive fundamental business transformation.",
  },
  {
    icon: <TrendingUp className="w-6 h-6" />,
    title: "Sector Specialization",
    desc: "Each partner carries 20+ years of domain expertise, not generalist capital allocation.",
  },
  {
    icon: <Users className="w-6 h-6" />,
    title: "Aligned Partnerships",
    desc: "We co-invest alongside management teams, ensuring complete alignment of interest and incentive.",
  },
];
...
export default function About() {
  return (
    <div className="pt-24 selection:bg-primary selection:text-primary-foreground" data-testid="about-page">
      ...
      {/* ── PHILOSOPHY ── */}
      <section className="py-28 md:py-36 bg-card" id="philosophy">
        <Reveal className="container mx-auto px-6 md:px-12 max-w-7xl">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            <motion.div variants={fadeInUp} className="lg:col-span-5">
              <p className="text-primary uppercase tracking-[0.2em] text-xs font-bold mb-4">Investment Philosophy</p>
              <h2 className="text-4xl md:text-5xl font-serif leading-tight">We build enduring value through asymmetric insight.</h2>
              <div className="mt-10 h-[1px] w-16 bg-primary/40" />
            </motion.div>
            <motion.div variants={fadeInUp} className="lg:col-span-6 lg:col-start-7 space-y-7 text-lg text-foreground/70 font-light leading-relaxed">
              <p>
                L-E Partners was founded on a singular premise: true value creation requires more than capital. It demands operational conviction, deep sector expertise, and the patience to execute multi-year strategic transformations.
              </p>
              <p>
                We do not participate in broad market cycles. Instead, we seek out complex, misunderstood, or transitioning assets where our unique operational toolkit unlocks disproportionate value for investors, management teams, and the businesses themselves.
              </p>
              <p>
                Our model is simple: make fewer investments, understand them more deeply, and stay involved longer. The result is a portfolio of businesses that are genuinely better companies because of our partnership — not just better-leveraged ones.
              </p>
            </motion.div>
          </div>
        </Reveal>
      ...
      <section className="py-28 md:py-36 border-t border-white/5">
        <Reveal className="container mx-auto px-6 md:px-12 max-w-7xl">
          <motion.div variants={fadeInUp} className="mb-16">
            <p className="text-primary uppercase tracking-[0.2em] text-xs font-bold mb-4">Core Principles</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {principles.map((p, i) => (
              <motion.div
                key={i}
                variants={fadeInUp}
                className="border border-white/8 p-10 group hover:border-primary/30 transition-colors"
                data-testid={`principle-${i}`}
              >
                <div className="text-primary mb-6 opacity-70 group-hover:opacity-100 transition-opacity">{p.icon}</div>
                <h3 className="text-2xl font-serif mb-3">{p.title}</h3>
                <p className="text-foreground/60 font-light leading-relaxed">{p.desc}</p>
              </motion.div>
            ))}
          </div>
        </Reveal>
      ...
      {/* ── TRACK RECORD ── */}
      <section className="py-28 md:py-36 bg-card border-t border-white/8" id="track-record" data-testid="track-record-section">
        <Reveal className="container mx-auto px-6 md:px-12 max-w-7xl">
          <motion.div variants={fadeInUp} className="text-center mb-20">
            <p className="text-primary uppercase tracking-[0.2em] text-xs font-bold mb-4">Track Record</p>
            <h2 className="text-4xl md:text-5xl font-serif">A legacy of compounding capital.</h2>
          </motion.div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-white/8 border border-white/8">
            {[
              ...
              <motion.div
                key={i}
                variants={fadeInUp}
                className="bg-card py-12 px-8 flex flex-col items-center justify-center text-center"
          ...
          <motion.p variants={fadeInUp} className="text-center text-xs text-foreground/30 mt-6 font-light tracking-wide">
      ...
      {/* ── SECTORS ── */}
      <section className="py-28 md:py-36 border-t border-white/5" id="sectors" data-testid="sectors-section">
        <Reveal className="container mx-auto px-6 md:px-12 max-w-7xl">
          <motion.div variants={fadeInUp} className="mb-16">
            <p className="text-primary uppercase tracking-[0.2em] text-xs font-bold mb-4">Focus Areas</p>
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
              <h2 className="text-4xl md:text-5xl font-serif max-w-2xl">Specialized focus drives outsized returns.</h2>
              <p className="text-foreground/55 max-w-md font-light md:text-right">
          ...
          <div className="space-y-px bg-white/8 border border-white/8">
            {sectors.map((s, i) => (
              <motion.div
                key={i}
                variants={fadeInUp}
                className="bg-background grid grid-cols-1 md:grid-cols-12 group hover:bg-card/60 transition-colors"
                data-testid={`sector-detail-${i}`}
              >
                <div className="md:col-span-3 p-10 md:p-12 border-b md:border-b-0 md:border-r border-white/8 flex flex-col justify-between">
                  <div>
                    <div className="text-primary mb-5 opacity-70 group-hover:opacity-100 transition-opacity">{s.icon}</div>
                    <h3 className="text-2xl font-serif mb-1 group-hover:text-primary transition-colors">{s.title}</h3>
                    <p className="text-xs uppercase tracking-widest text-primary/60 font-semibold">{s.tagline}</p>
                  </div>
                </div>
                <div className="md:col-span-5 p-10 md:p-12 md:border-r border-white/8">
                  <p className="text-foreground/65 font-light leading-relaxed">{s.desc}</p>
                </div>
                <div className="md:col-span-4 p-10 md:p-12">
                  <p className="text-xs uppercase tracking-widest text-foreground/40 font-semibold mb-5">Focus Areas</p>
                  <ul className="space-y-2">
                    {s.examples.map((ex, j) => (
                      <li key={j} className="flex items-center gap-3 text-sm text-foreground/60 font-light">
